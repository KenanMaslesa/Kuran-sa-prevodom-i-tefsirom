...
const HOUR_IN_SECONDS = 60 * 60; //60 seconds * 60 = 1 hour
const DAY_IN_SECONDS = 24 * HOUR_IN_SECONDS;
const MONTH_IN_SECONDS = 31 * DAY_IN_SECONDS;
const YEAR_IN_SECONDS = 12 * MONTH_IN_SECONDS;

// Expire time in seconds
const TTL = YEAR_IN_SECONDS * 10; //10 years
...
export class CachingService {
  ...
  cacheRequest(url, data): Promise<any> {
    const validUntil = (new Date().getTime()) + TTL * 1000; //milliseconds
    url = `${CACHE_KEY}${url}`;
